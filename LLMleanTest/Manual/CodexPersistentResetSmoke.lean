...
def tracePath : System.FilePath :=
  "/tmp/llmlean-codex-persistent-reset-smoke.jsonl"
...
def baseOptions (cwd : System.FilePath) : LLMlean.Codex.Session.Options := {
  cwd := cwd,
  debug := true,
  readTimeoutMs := 5000,
  turnTimeoutMs := 5000,
...
#eval do
  ...
  assertNatEq "process starts after reset" 2 (countContaining "START" lines)
  assertNatEq "initialize requests after reset" 2 (countContaining "\"method\":\"initialize\"" lines)
  assertNatEq "thread/start requests after reset" 2 (countContaining "\"method\":\"thread/start\"" lines)
  assertNatEq "turn/start requests after reset" 2 (countContaining "\"method\":\"turn/start\"" lines)

  IO.println "persistent Codex reset smoke passed"
